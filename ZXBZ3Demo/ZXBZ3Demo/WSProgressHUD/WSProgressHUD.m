...
static UIImage *WSProgressHUDWarningImage;
...
{
    [self showWithStatus:string maskType:WSProgressHUDMaskTypeClear];
}
...
{
    [[self shareInstance] addOverlayViewToWindow];
    [[self shareInstance] setProgressHUDIndicatorStyle:WSProgressHUDIndicatorSmallLight];
...
{
    [self showImage:WSProgressHUDWarningDefaultImage() status:string];
}

+ (void)showTextWithStatus:(nullable NSString *)string
{
...
//                    }
                ...
                self.labelView.center = CGPointMake(hudCenterX, hudCenterY + 24);
                self.spinnerView.center = self.indefiniteAnimationView.center = self.indicatorView.center = CGPointMake(hudCenterX, hudCenterY-WSProgressHUDStringRect.size.height/2);
...
- (CGSize)hudSizeWithString:(NSString *)string
{
    ...
    WSProgressHUDDefaultHeight = 80;
    WSProgressHUDDefaultWidth = 80;
    ...
    switch (self.hudType) {
        case WSProgressHUDTypeStatus: {
            ...
            if (string) {
                ...
                WSProgressHUDDefaultHeight = stringHeight + WSProgressHUDDefaultHeight - 10;
                WSProgressHUDDefaultWidth = MAX(WSProgressHUDDefaultWidth, WSProgressHUDDefaultHeight);
        ...
        case WSProgressHUDTypeImage: {
            ...
            if (self.imageView.image) {
                ...
                WSProgressHUDDefaultHeight = stringHeight + WSProgressHUDHeightEdgeOffset + 10;
                WSProgressHUDDefaultWidth = stringWidth + WSProgressHUDImageTypeWidthEdgeOffset + 20;
            }
...
- (NSTimeInterval)displayDurationForString:(NSString *)string {
    CGFloat duration = MIN((CGFloat)string.length*0.08 + 0.5, 5.0);
    if (duration < 1.0) {
        duration = 1.0;
    }
    return duration;
...
CG_INLINE UIImage * WSProgressHUDWarningDefaultImage()
{
    if (WSProgressHUDWarningImage == nil) {
        UIImage *failurImage = WSProgressHUDImageWithName(@"warning@2x", @"png");
        WSProgressHUDWarningImage = WSImageByAddTintColr(failurImage, WSProgressHUDForeGroundDefaultColor());
    }
    return WSProgressHUDWarningImage;
}
